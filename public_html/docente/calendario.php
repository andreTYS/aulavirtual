...
$sesiones = $pdo->prepare(
    "SELECT s.fecha, s.hora_inicio, s.tema, s.estado, c.nombre AS curso_nombre, c.id AS curso_id
     ...
     WHERE c.docente_id = :docente_id AND s.fecha BETWEEN :inicio AND :fin AND s.estado != 'cancelada'
     ORDER BY s.hora_inicio"
);
$sesiones->execute(['docente_id' => $docenteId, 'inicio' => $monthStart->format('Y-m-d'), 'fin' => $monthEnd->format('Y-m-d')]);
...
$tareas = $pdo->prepare(
    "SELECT t.fecha_limite, t.titulo, c.nombre AS curso_nombre, c.id AS curso_id
     ...
     WHERE c.docente_id = :docente_id AND DATE(t.fecha_limite) BETWEEN :inicio AND :fin"
);
$tareas->execute(['docente_id' => $docenteId, 'inicio' => $monthStart->format('Y-m-d'), 'fin' => $monthEnd->format('Y-m-d')]);
...
foreach ($sesiones as $s) {
    ...
    $eventosPorDia[$dia][] = [
        ...
        'link' => '/docente/curso.php?id=' . (int) $s['curso_id'],
    ];
}
foreach ($tareas as $t) {
    ...
    $eventosPorDia[$dia][] = [
        ...
        'link' => '/docente/curso.php?id=' . (int) $t['curso_id'],
    ];
}

$diasEnMes = (int) $monthEnd->format('j');
$primerDiaSemana = (int) $monthStart->format('N'); // 1 (lunes) .. 7 (domingo)
$hoy = date('Y-m-d');
...
<div class="av-card">
    <div class="av-cal-head">
        <a href="/docente/calendario.php?month=<?= $prevMonth ?>" class="av-btn av-btn--outline av-btn--sm">&larr; Anterior</a>
        <h3><?php
            $meses = ['', 'enero', 'febrero', 'marzo', 'abril', 'mayo', 'junio', 'julio', 'agosto', 'septiembre', 'octubre', 'noviembre', 'diciembre'];
            echo e($meses[(int) $monthStart->format('n')] . ' ' . $monthStart->format('Y'));
        ?></h3>
        <a href="/docente/calendario.php?month=<?= $nextMonth ?>" class="av-btn av-btn--outline av-btn--sm">Siguiente &rarr;</a>
    </div>
